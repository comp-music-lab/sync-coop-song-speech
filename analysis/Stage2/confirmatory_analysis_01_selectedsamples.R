...
for(i in 1:2) {
  ...
  print(rhat_theta[[i]])
  write.csv(rhat_theta[[i]], sprintf("./figure/exploratory_analysis_hguess34_01_stats_%d.csv", i), row.names=TRUE)
...
write.csv(lnZ, "./figure/exploratory_analysis_hguess34_01_lnZ.csv", row.names=FALSE)
...
ggsave(g, dpi=1200, height=6, width=9, filename="./figure/exploratory_analysis_hguess34_01_BF.png")

plot(g)

for (i in 1:2) {
  g <- traceplot(modellist[[i]], pars=c("sgm", "s_1", "s_2", "be", "g"), inc_warmup=TRUE, nrow=2)
  figfilename <- paste("./figure/exploratory_analysis_hguess34_01_chain_", i, ".png", sep="")
...
for(j in 1:4) {
  ...
  ggplotlist[[j]] <- g
  ggsave(filename=paste("./figure/exploratory_analysis_hguess34_01_PPC_", j, ".png", sep=""),
...
ggsave(filename="./figure/exploratory_analysis_hguess34_legend.png", plot=gl, width=12, height=4)
plot(gl)