...
write.csv(lnZ,
          here(herepath, "interim_output", "confirmatory_analysis_02_lnZ.csv"),
          row.names=FALSE)
...
figfilename <- here(herepath_fig, "confirmatory_analysis_02_1_BF.png")
...
figfilename <- here(herepath_fig, "confirmatory_analysis_02_2_BF.png")
ggsave(g, dpi=1200, height=6, width=9, filename=figfilename)

# Plot original chain samples
for (i in 1:length(pattern)) {
  g <- traceplot(modellist[[i]], pars=c("sgm", "s_1", "s_2", "be", "g"), inc_warmup=FALSE, nrow=2)
  figfilename <- here(herepath, "interim_output", sprintf("confirmatory_analysis_02_chain_%d.png", i))
  ggsave(g, dpi=1200, height=6, width=9, filename=figfilename)
}
...
for (i in 1:length(pattern)) {
  ...
  figfilename <- here(herepath, "interim_output", sprintf("confirmatory_analysis_02_conschain_%d.png", i))
  ggsave(g, dpi=1200, height=6, width=9, filename=figfilename)
}