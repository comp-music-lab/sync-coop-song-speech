...
write.csv(lnZ,
          here(herepath, "interim_output", "confirmatory_analysis_01_lnZ.csv"),
          row.names=FALSE)
...
for(j in 1:4) {
  ...
  figfilename <- here(herepath, "interim_output", sprintf("confirmatory_analysis_01_PPC_%d.png", j))
  ggsave(filename=figfilename, plot=ggplotlist[[j]], width=4, height=2)
}