...
### Onle keep participants who gueesed 3 and 4 ###
source("h_datalist34.R")
datalist <- h_datalist34("./mv2_cleaned_coding_merged.csv", datalist)
...
for(i in c(2, 4, 1, 3)) {
  ...
  print(rhat_theta[[i]])
  write.csv(rhat_theta[[i]], sprintf("./figure/exploratory_analysis_hguess34_02_stats_%d.csv", i), row.names=FALSE)
...
write.csv(lnZ, "./figure/exploratory_analysis_hguess34_02_lnZ.csv", row.names=FALSE)
...
ggsave(g, dpi=1200, height=6, width=9, filename="./figure/exploratory_analysis_hguess34_02_1_BF.png")
...
ggsave(g, dpi=1200, height=6, width=9, filename="./figure/exploratory_analysis_hguess34_02_2_BF.png")

# Plot original chain samples
for (i in 1:length(pattern)) {
  g <- traceplot(modellist[[i]], pars=c("sgm", "s_1", "s_2", "be", "g"), inc_warmup=FALSE, nrow=2)
  figfilename <- paste("./figure/exploratory_analysis_hguess34_02_chain_", i, ".png", sep="")
...
for (i in 1:length(pattern)) {
  ...
  figfilename <- paste("./figure/exploratory_analysis_hguess34_02_conschain_", i, ".png", sep="")
  ggsave(g, dpi=1200, height=6, width=9, filename=figfilename)
}