...
for(counter in 1:100) {
  for(i in 1:2) {
    ...
    print(rhat_theta[[i]])
    write.csv(rhat_theta[[i]], sprintf("./figure_he/confirmatory_analysis_01_stats_%d_%d.csv", i, counter), row.names=TRUE)
  ...
  write.csv(lnZ, sprintf("./figure_he/confirmatory_analysis_01_lnZ_%d.csv", counter), row.names=FALSE)
  saveRDS(
    possamplelist, 
    file=sprintf("./figure_he/confirmatory_analysis_01_possample_%d.rds", counter)
  )
}